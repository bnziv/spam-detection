  ...
  return (
    <div className="flex items-center justify-center min-h-screen p-4 flex-col">
      <div className="w-full max-w-md bg-white shadow-md rounded-lg p-6">
        ...
        ${result === 'spam' ? 'text-yellow-500' : result === 'ham' ? 'text-green-500' : result === 'phishing' ? 'text-red-500' : 'text-gray-500'}`}>
          Spam Detection
        </h1>

        <textarea
          className="w-full h-32 p-3 mt-4 border border-gray-300 rounded-lg focus:ring focus:ring-blue-300 resize-none"
      ...
      {result && (
          ...
          ${result === 'spam' ? 'bg-yellow-500' : result === 'ham' ? 'bg-green-500' : result === 'phishing' ? 'bg-red-500' : 'bg-gray-500'}`}>
            {result.toUpperCase()} {(probability[result] * 100).toPrecision(2)}%
            
            {/* Display other probabilities if the main one is less than 90% */}
            {probability[result] < 0.9 &&
            Object.entries(probability).map(([label, prob]) => (
              label !== result &&
              <div key={label}>
                {label.toUpperCase()}: {(prob * 100).toPrecision(2)}%
              </div>
            ))
            }
          </div>
